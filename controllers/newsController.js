...
class newsController {
    add_news = async (req, res) => {
        ...
        try {
            ...
            if (files.image) {
                const uploadResult = await cloudinary.uploader.upload(files.image[0].filepath, { folder: 'news_images' });
                imageUrl = uploadResult.url; // Get URL from uploaded image
            }

            // Save news data with image URL (not in description)
            const news = await newsModel.create({
                writerId: id,
                title: title[0].trim(),
                slug: title[0].trim().split(' ').join('-'),
                category,
                description: description[0], // Keep description clean without image URL
    ...
    get_categories = async (req, res) => {
        try {
            const categories = await newsModel.aggregate([
                // Match only news with 'active' status
                {
                    $match: {
                        status: 'active'
                    }
                },
                // Group by category and count the number of active news per category
                {
                    $group: {
                        _id: '$category',
                        count: { $sum: 1 }
    ...
    // Phương thức lấy thống kê cho dashboard
